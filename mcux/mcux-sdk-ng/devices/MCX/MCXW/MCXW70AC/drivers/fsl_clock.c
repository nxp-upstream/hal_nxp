...
uint32_t CLOCK_GetFreq(clock_name_t clockName)
{
    ...
    switch (clockName)
    {
        ...
        case kCLOCK_CoreSysClk:
            ...
            break;
        case kCLOCK_PlatClk:
            freq = CLOCK_GetSysClkFreq(kSCG_SysClkPlatform);
            break;
        case kCLOCK_BusClk:
...
status_t CLOCK_InitSysOsc(const scg_sosc_config_t *config)
{
    ...
    CLOCK_REG(&SCG_0->SOSCCSR) = (uint32_t)config->enableMode | SCG_SOSCCSR_SOSCEN_MASK;
...
{
    if ((CLOCK_REG(&SCG_0->SOSCCSR) & SCG_SOSCCSR_SOSCVLD_MASK) ==
...
status_t CLOCK_InitSirc(const scg_sirc_config_t *config)
{
    ...
    CLOCK_REG(&SCG_0->SIRCCSR) = (uint32_t)config->enableMode;
     ...
     * $ref clock_c_ref_2$.
     */
    while ((CLOCK_REG(&SCG_0->SIRCCSR) & SCG_SIRCCSR_SIRCVLD_MASK) != SCG_SIRCCSR_SIRCVLD_MASK)
    {
    }
...
{
    uint32_t reg = CLOCK_REG(&SCG_0->SIRCCSR);
...
uint32_t CLOCK_GetSircFreq(void)
{
     ...
     * $ref clock_c_ref_2$.
     */
    if ((CLOCK_REG(&SCG_0->SIRCCSR) & SCG_SIRCCSR_SIRCVLD_MASK) == SCG_SIRCCSR_SIRCVLD_MASK) /* SIRC is valid. */
    {
...
status_t CLOCK_InitFirc(const scg_firc_config_t *config)
{
    ...
    CLOCK_REG(&SCG_0->FIRCCFG) = SCG_FIRCCFG_RANGE(config->range);

    if (config->trimConfig != NULL)
    {
        CLOCK_REG(&SCG_0->FIRCTCFG) =
        ...
        /* Set trim mode. */
        CLOCK_REG(&SCG_0->FIRCCSR) = (uint32_t)config->trimConfig->trimMode;

        /*
         * $Branch Coverage Justification$
         * (CLOCK_REG(&SCG_0->FIRCCSR) & SCG_FIRCCSR_FIRCERR_MASK) == SCG_FIRCCSR_FIRCERR_MASK
         * not covered.
         * $ref clock_c_ref_3$.
         */
        if ((CLOCK_REG(&SCG_0->FIRCCSR) & SCG_FIRCCSR_FIRCERR_MASK) == SCG_FIRCCSR_FIRCERR_MASK)
        {
            return (status_t)kStatus_Fail;
        }
    }

    CLOCK_REG(&SCG_0->FIRCCSR) |= (SCG_FIRCCSR_FIRCEN_MASK | (uint32_t)config->enableMode);

    /* Wait for FIRC clock to be valid. */
    while ((CLOCK_REG(&SCG_0->FIRCCSR) & SCG_FIRCCSR_FIRCVLD_MASK) != SCG_FIRCCSR_FIRCVLD_MASK)
    {
    }
...
{
    uint32_t reg = CLOCK_REG(&SCG_0->FIRCCSR);
...
status_t CLOCK_InitRosc(const scg_rosc_config_t *config)
{
    ...
    while ((CLOCK_REG(&SCG_0->ROSCCSR) & SCG_ROSCCSR_ROSCVLD_MASK) != SCG_ROSCCSR_ROSCVLD_MASK)
    {
    }
...
{
    if ((CLOCK_REG(&SCG_0->ROSCCSR) & SCG_ROSCCSR_ROSCVLD_MASK) ==
...
uint32_t CLOCK_Get1MClkFreq(void)
{
    ...
    {
        if ((reg & MRCC_RSTB_MASK) != MRCC_RSTB_MASK)
        {
            /* Clock 1M is held in reset */
            return 0U;
        }
