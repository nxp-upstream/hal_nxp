...
#define __EDMA_STEP_BY_CH(ch)                                                          \
    ( ((unsigned)(ch) < 16u)                                                           \
        /* Nhóm 0..15: mỗi block 4 kênh tăng 0x10000; trong block: 0,0x2000,0x8000,0xA000 */ \
        ? ( ((unsigned)(ch) >> 2) * 0x10000u +                                         \
            ( ((unsigned)(ch) & 3u) == 0u ? 0x0000u :                                  \
              ((unsigned)(ch) & 3u) == 1u ? 0x2000u :                                  \
              ((unsigned)(ch) & 3u) == 2u ? 0x8000u : 0xA000u ) )                      \
        /* Nhóm 16..31: vùng cơ sở 0x600000, cộng theo cặp kênh + phần lẻ/chẵn */      \
        : ( 0x600000u +                                                                \
            ((((unsigned)(ch) - 16u) & ~1u) << 14) /* mỗi cặp +0x8000 */               \
          + ((((unsigned)(ch) - 16u) & 1u)  << 13) /* kênh lẻ +0x2000 */               \
          + 0x0C000u ) )
