...
#endif
            /* Disconnect from AP/GO if station/GC is associated. */
            if (is_sta_connecting() || is_wfd_gc_connected())
            {
                wlan_disconnect();
                while (!is_sta_idle() || !is_wfd_idle())
                {
                    OSA_TimeDelay(1000);
                }
            }
...
#if UAP_SUPPORT
            net_wlan_set_mac_address(&wlan.sta_mac[0], &wlan.uap_mac[0], NULL);
#else
